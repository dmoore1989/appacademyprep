...
class String
  def caesar(shift)
    ...
    shift_string = []
    each_char do |letter|
      char_string << letter.ord
    end
    
    char_string.each_with_index do |letter, index|
      shift_string << letter + shift
      if (shift_string[index] > 122) || (letter <= 90 && shift_string[index] > 90)
        shift_string[index] -= 26
      end
    end
    
    char_string = []
    shift_string.each { |letter| char_string << letter.chr }
    char_string.join('')
  end
end
...
class Fixnum
  def stringify(base)
    ...
    string = ''
    exponent = 0
    while self >= base ** exponent
      character = (((self / base ** exponent) % base))
      if character >= 10
        character = string_hash[character.to_s]
      end
      string = character.to_s + string
      exponent += 1
    end
    string
  end
